...
| Tab | What it does |
|-----|--------|
| **Overview** | The day's cross-source digest (a 2–3 sentence "today at a glance" hero) plus "Today's Top 10" — both pre-generated by the pipeline. Items the AI flags as "breaking" get a Breaking badge and a distinct style, sorted to the top. Open and read, zero waiting. |
| **Summary** | The day's AI bullet points (in Chinese, pre-generated by the pipeline) for 8 archived sources; tap an entry to open the original article in the built-in WebView. Instant, no waiting. |
| **Trends** | Cross-source keyword leaderboard computed from the last 14 days of snapshots (pure statistics, no AI): window hit counts, a mini trend line, and up/down markers; tap a keyword to expand its representative articles. The top bar also hosts the entry to My Follows (keyword subscriptions). |
| **More** | Settings, AI service config, source hub, favorites (read-later), archive history, AI Daily archive, About, etc. |

The three content tabs (Overview / Summary / Trends) support **pull-to-refresh**, which bypasses the archive cache to pick up the pipeline's latest batch right away.
...
- **In-app update check & install** (About page) against GitHub Releases — the "new version" dialog downloads the APK with live progress (cancellable) and hands off to the system installer, falling back to the Release page on failure; plus `ainewshub://` deep links (`web?url=…`, `tab/<overview|summary|trends|more>`, `settings`) for browsers, QR codes and automation tools
...
app/                       the single Android module
  src/main/java/com/peng/ainewshub/
    ...
    ui/                    ViewModel + Compose Screen, split by feature
      ...
      follows/             "My Follows" keyword-subscription feed (Trends top bar entry)
