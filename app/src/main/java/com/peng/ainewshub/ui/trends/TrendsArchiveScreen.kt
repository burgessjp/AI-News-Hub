...
/**
 * 历史热词 —— 可选日期列表(根级独立索引 `trends_history.json` 的键,倒序)。
 *
 * 点击某天进入该日的热词榜页([TrendsDateScreen],复用趋势 Tab 内容渲染,
 * 榜单同样带当日的排名变化标记)。视觉对齐「历史总览/历史摘要」日期列表:
 * 左栏相对日期 + 周几,行间发丝线;无右栏计数(热词榜是跨源单份产物)。
 *
...
@Composable
fun TrendsArchiveScreen(
    onSelectDate: (String) -> Unit,
    onBack: () -> Unit,
    listState: LazyListState,
    vm: TrendsArchiveViewModel = viewModel()
) {
    ...
    Scaffold(
        ...
        topBar = {
            AppTopBar(
                title = stringResource(R.string.trends_archive_title),
...
/**
 * 日期条目 —— 与历史总览/历史摘要日期行同语言:左栏相对日期(今天/昨天/前天/
 * M月d日 · 周X),右缘 chevron;无右栏计数(热词榜为跨源单份产物)。
 */
@Composable
