package com.peng.ainewshub.ui.anim
 ...
 *
 * @property PUSH 普通二级页(Search / Settings / Detail / Web 等):横向位移 + 淡化。
 *  Web 页曾因 WebView 位移撕裂单独 override 为 FADE,实测无此问题后已统一 PUSH。
 * @property NONE tab 根页之间切换:带 stagger 的 crossfade(防中间变暗)。
 */
enum class PageNavStyle { PUSH, NONE }
 ...
 *
 * 1. **两边都是 NONE(tab↔tab)→ 带 stagger 的 crossfade。**
 *    旧页先退(150ms Accel)、新页稍迟后进(300ms Decel)——错峰避免两者在
 *    ~50% 透明度时叠加成"中间变暗"。纯 fade,无位移。
 *
 * 2. **其余(NONE↔PUSH / PUSH↔PUSH)→ 横向推入 + 淡化,方向由 [back] 决定。**
...
fun pageTransition(enter: PageNavStyle, exit: PageNavStyle, back: Boolean = false): ContentTransform = when {
    ...
    // ===== 规则 2:PUSH 类横向推入 + 淡化,方向由 back 决定 =====
...
fun predictivePopTransition(
    ...
    // ===== 退出页滑向手势一侧边缘,缩小+淡出;下级页面从对侧 1/4 处淡入 =====
