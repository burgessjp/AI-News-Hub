...
- **语音速报**：总览页与关注页顶栏播报入口，朗读当日综述 + Top10 点评、关注流命中条目（通勤/睡前场景）；总览播报优先播放数据流水线预生成的神经语音（MOSS-TTS-Nano，真暂停/原地续播），不可用时自动回落系统 TTS；前台服务通知栏提供上一条/暂停/下一条/停止控制，背景音乐压低而非打断；零新增依赖
