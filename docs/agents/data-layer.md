...
- **语音速报预生成音频**（总览播报优先通道）：读 `index.json` 顶层 `latest_audio` 字段（流水线 `tts_broadcast.py` 用 MOSS-TTS-Nano 按当日总览预合成的 MP3 清单；`generatedAt` 与 `latest_overview.generatedAt` 严格同值，App 侧据此判定新鲜度——清单缺失 / 批次不一致 / 条数与本地播放列表不符 → 回落系统 TTS + Toast 提示，预生成音频是增强项不是依赖项）。App 端 `BroadcastRepository` 只反序列化（经 `ArchiveHttpClient.fetchLatestAudio`，与 index 共享 2 分钟缓存与磁盘兜底）；音频本体由 `TtsPlaybackService` 经 `ArchiveHttpClient.audioUrl(relPath)`（REST API raw 端点拼法，WAF 合规）交 `AudioEntryPlayer`（MediaPlayer）流式播放，真暂停/原地续播，单条播放失败回落系统 TTS 重读该条（两通道文本同规则，用户无感）；关注页播报恒系统 TTS（个性化内容无法预生成）。
