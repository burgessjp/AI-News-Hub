...
- WorkManager one-shot 自查链轮询归档 `latest_overview.generatedAt` 指纹（不用 `updated_at_ms`，总览失败继承旧值时不误报），检查时刻表对齐流水线批次（北京时间 08:40 / 18:40 / 22:40 = 各批次 +40min 余量，**改流水线批次时间必须同步改 `CHECK_SLOTS`**；批次 08:00 / 18:00 由仓库外机器调度，仓库 workflow 仅承载 22:00 批——勿因 workflow 只有一个 cron 误判 CHECK_SLOTS 为 bug）；档内未就绪 40min 补查最多 2 次。
