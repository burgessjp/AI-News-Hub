...
- 配色直接取 `ui/theme/Color.kt` 设计令牌组 day/night `ColorProvider`（App 迷你版，不用壁纸动态色）；**皮肤跟随 App 设置**：`provideGlance` 为 suspend，直接读 `SettingsStore.prefsFlow` 取 `AppSkin` 经 `widgetColors(mono)` 构造两套颜色组（Classic 绑顶层令牌；Mono 内联 `MonoLight/MonoDarkColors` 对应槽位，error 系刻意沿用 Classic 红），卡面/渐变头 drawable 按「皮肤 resId 分支（`*_mono` 变体）× 明暗 day/night 限定符」四象限选择（同 `BrandWordmark` 做法）；切皮肤时 `AiNewsHubApp.onSelectSkin` 主动 `updateAll` 即时重绘；条目只展示排名 + 标题（+突发胶囊），来源/互动指标刻意不上小组件；Glance 1.1.1 不支持 res/font 自定义字体，层级靠字号 + 字重。
- 视觉对齐 App 内「今日热点」卡（`HotTopicsSection`）：头部品牌渐变 Hero（经典 `widget_header_gradient` / 纸墨 `widget_header_gradient_mono`，day/night + 皮肤四象限，BrandGradient 同源）= 标题行 + 「今日综述」digest 正文（≤2 行截断，空串不渲染，与总览 Tab digest Hero 同构；digest 随 items 一同存入 `hot_now_widget` SharedPreferences 的 `digest` 键）+ 卡面色描边背景（经典 `widget_bg` / 纸墨 `widget_bg_mono` day/night drawable，surfaceContainerLow + 1dp outlineVariant 描边）；条目为迷你排名徽章（18dp，分档同 App RankBadge：1 名 tertiary 实心 / 2-3 tertiaryContainer / 其余 surfaceContainerHigh）+ Bold 标题，行间发丝线。改徽章分档/渐变色须与 `ui/components/RankBadge.kt`、`theme/Color.kt`（含 Mono 色板）保持同步。
