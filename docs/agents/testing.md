...
- **布局**：`scripts/tests/` 按「被测对象_层次」分文件：`test_common` / `test_fetch_pure` 纯函数 / `test_fetch_io` 落盘扫描 / `test_fetch_index` 继承语义核心 / `test_fetch_main` 端到端 / `test_sources_registry` 注册表护栏（SOURCES 序与 SOURCE_KEYS 集合相等、EMPTY_OK 子集、fetch_with_retry 契约冒烟）/ `test_trend_pure`+`test_trend_generate`+`test_trend_refine`+`test_trend_write` 趋势统计全链（提取归一/门槛护栏/AI 精修/归档涨跌）/ `test_push_data` 推送（脱敏、overlay、重试矩阵 + 本地 bare 仓 e2e）/ `test_tts_broadcast` 语音速报（GGUF 双闸、合成降级、latest_audio、`__main__` 兜底）。fixtures 复用同一套裁剪规约（真实批次、latest 指针保真、items ≤5），来源 `out/` 本地产物或 `repo/` 克隆——**绝不把 `repo/` 的 remote URL 写进测试或 fixture（内嵌 token）**。
...
- 测试名与注释惯例与 App 侧一致：中文直述被钉住的契约。fetch_data / sources / trend_keywords / push_data / tts_broadcast 已全覆盖（155 用例）；`overview_summary.py` / `ai_summary.py` / `backfill_*.py` 暂不覆盖（AI 两层有间接覆盖，backfill 是一次性工具且复用已测 I/O 层）。
